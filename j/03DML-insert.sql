-- 고객테이블의 모든 컬럼에 회원정보 추가
insert into CUSTOMERS
values ('strayberry', '최유경', 30, 'vip', '공무원', 100);

-- 고객테이블의 일부 컬럼에 회원정보 추가
insert into  CUSTOMERS (cid, name, age, job)
values ('pear', '채광주', 31, '학생');




-- 열 수정 방법
alter table CLOUDS.CUSTOMERS
modify age int null;
...
-- 주문 테이블에 데이터 추가
-- insert into 테이블명
-- select 값1,갑2,값3,값4 from dual
-- union all select 값1,갑2,값3,값4 from dual
insert into ORDERS
          ...
          union all select 'o04', 'carrot', 'p02', 8, '부산시 금정구', '2022-02-01'from dual
          union all select 'o05', 'melon', 'p06', 36, '경기도 용산시', '2022-02-20'from dual
...
--oracle 21c, MySQL, PostgreSQL 공통
...
insert into ORDERS
values ('o05', 'melon', 'p06', 36, '경기도 용산시', '2022-02-20');
insert into ORDERS
...
insert into ORDERS values
    ...
    ('o05', 'melon', 'p06', 36, '경기도 용산시', '2022-02-20'),
...
create table seoul_foods (
 개방자치단체코드  clob,   관리번호      clob,  인허가일자     clob, 인허가취소일자   clob,
 영업상태코드    clob,     영업상태명     clob,  상세영업상태코드  clob,상세영업상태명   clob,
 폐업일자      clob,   휴업시작일자    clob,      휴업종료일자    clob,  재개업일자     clob,
 전화번호      clob,            소재지면적     clob,    소재지우편번호   clob,     지번주소      clob,
 도로명주소     clob,        도로명우편번호   clob,      사업장명      clob,        최종수정일자    clob,
 데이터갱신구분   clob,         데이터갱신일자   clob,        업태구분명     clob,      "좌표정보(X)" clob,
 "좌표정보(Y)" clob,                위생업태명     clob,         남성종사자수    clob,여성종사자수    clob,
 영업장주변구분명  clob,                 등급구분명     clob,     급수시설구분명   clob,   총인원       clob,
 본사종업원수    clob,                공장사무직종업원수 clob,   공장판매직종업원수 clob,공장생산직종업원수 clob,
 건물소유구분명   clob,                보증액       clob,       월세액       clob,다중이용업소여부  clob,
 시설총규모     clob,                전통업소지정번호  clob,      전통업소주된음식  clob,   홈페이지      clob
);
...
select * from accident2016;


delete from accident2016;
